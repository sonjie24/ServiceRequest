...
{
    public class ConsumerTokenManager : IConsumerTokenManager<ConsumerAccountEntity>
    {
        ...
        public ConsumerTokenManager(ILogger<ConsumerTokenManager> _logger,
        ...
        public async Task<AuthenticationResult> GenerateAuthenticationAsync(ConsumerAccountEntity user)
        {
            try
            {
                ...
                var refreshToken = new RefreshTokenConsumer
                {
                    JwtId = token.Id,
                    userGuid = user.GUID,
                ...
                await database.context.RefreshTokenConsumers.AddAsync(refreshToken);
        ...
        public async Task<ResultViewModel> RefreshTokenAsync(string accessToken, string refreshToken)
        {
            ...
            try
            {
                ...
                var storedRefreshToken = await database.context.RefreshTokenConsumers.SingleOrDefaultAsync(predicate: x => x.Token == refreshToken);
                ...
                database.context.RefreshTokenConsumers.Update(storedRefreshToken);
                await database.context.SaveChangesAsync();

                string userId = validateToken.Claims.Single(x => x.Type == "id").Value;
                var user = await database.context.ConsumerAccounts.SingleOrDefaultAsync(x => x.GUID == userId);

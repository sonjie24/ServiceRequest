...
{
    public class AdminTokenManager  :IAdminTokenManager<AdminAccountEntity>
    {
        ...
        public AdminTokenManager(ILogger<AdminTokenManager> _logger,
        ...
        public async Task<AuthenticationResult> GenerateAuthenticationAsync(AdminAccountEntity user)
        {
            try
            {
                ...
                await database.context.RefreshTokenAdmins.AddAsync(refreshToken);
        ...
        public async Task<ResultViewModel> RefreshTokenAsync(string accessToken, string refreshToken)
        {
            ...
            try
            {
                ...
                var storedRefreshToken = await database.context.RefreshTokenAdmins.SingleOrDefaultAsync(predicate: x => x.Token == refreshToken);
                ...
                database.context.RefreshTokenAdmins.Update(storedRefreshToken);
                await database.context.SaveChangesAsync();

                string userId = validateToken.Claims.Single(x => x.Type == "id").Value;
                var user = await database.context.AdminAccounts.SingleOrDefaultAsync(x => x.GUID == userId);
